...
#' Random Sequentially Spectral initialization for FTSVD. 
#' @param f.tensor An order-d tabular tensor.
#' @param f.grid A list recording the grids for all functional modes.
...
FTSVD.random <- function(f.tensor, f.grid, rank, penalty){
  # Check the input validity.
  if(class(f.tensor) != "Tensor") stop("invalid input: f.tensor should be of tensor type.")
  p = dim(f.tensor)
  d = length(p)
  ...
  for (k in 1:d) {
    ...
    if (is.null(f.grid[[k]])){
      if (k==1){
        ...
        uk = svd(k_unfold(Y, k)@data)$u[,1:rank] %*% svd(matrix(rnorm(rank),rank,1))$u
      }
      else{
    ...
    # functional mode: ERM
      ...
      temp = svd(k_unfold(Y, k)@data)
      uk = temp$d[1] * temp$u[,1]
      beta = Kers.inverse[[k]] %*% uk
      xi = Kers[[k]] %*% beta
      xi = xi / norm(xi, type = 'F')
      Y = ttm(Y, t(xi), k)
      init = c(init, list(as.vector(xi)))
    }
  }
  return(init)
}
...
FTSVD <- function(f.tensor, f.grid, rank, resolution, penalty, L = 20, initialization = 'Random'){
  # Check the input validity.
  if(class(f.tensor) != "Tensor") stop("invalid input: f.tensor should be of tensor type.")
  p = dim(f.tensor)
  d = length(p)
  ...
  if (rank==1){
    ...
    # obtain the iterated result from random initializations
    if (initialization == 'Random'){
      lambda.set = rep(0,L)
      loadings.set = rep(list(NULL),L)
      for (l in 1:L) {
        l.init = FTSVD.random(f.tensor, f.grid, rank, penalty)
        l.est = FTSVD.iteration(f.tensor, f.grid, l.init, resolution, penalty)
        lambda.set[l] = l.est[[1]]
        loadings.set[[l]] = l.est[2:(d+1)]
      }
