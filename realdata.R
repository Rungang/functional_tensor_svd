...
res = FTSVD(data.interpolate, f.grid = c(list(NULL),list(NULL),list(tn/24)), 
                  rank=3, L=50, penalty = 1e-5, initialization = "Spectral")
...
agg_obs = data.frame(mean = mean, se = sd, Month = rep(tn, 4), 
                     Component = rep(c("Component 2", "Component 3"), each = length(tn)*2),
                       Delivery = as.factor(rep(c("Vaginal","Cesarean","Vaginal","Cesarean"),each=length(tn))))
...
p_feat_obs2 <- ggplot(data=agg_obs_2, 
                      aes(x=Month, y=mean, group=Delivery, color=Delivery)) + 
  geom_line(size=1) + ggtitle('Component 2') + 
  geom_ribbon(aes(ymin=mean-qnorm(0.95)*se, ymax=mean+qnorm(0.95)*se, 
                  color=Delivery, fill=Delivery), linetype=2, alpha=0.3) +
  theme(text = element_text(size = 15), axis.title.y = element_blank())

mean_3 = c(apply(agg_feature_3[which(metauni$delivery=="Vaginal"),], 2, mean),
           apply(agg_feature_3[which(metauni$delivery=="Cesarean"),], 2, mean))
sd_3 = c(apply(agg_feature_3[which(metauni$delivery=="Vaginal"),], 2, FUN=function(x){sd(x)/sqrt(length(x))}),
         apply(agg_feature_3[which(metauni$delivery=="Cesarean"),], 2, FUN=function(x){sd(x)/sqrt(length(x))})) 

agg_obs_3 = data.frame(mean = mean_3, se = sd_3, Month = rep(tn, 2), 
                       Delivery = as.factor(rep(c("Vaginal","Cesarean"),each=length(tn))))
p_feat_obs3 <- ggplot(data=agg_obs_3, 
                      aes(x=Month, y=mean, group=Delivery, color=Delivery)) + 
  geom_line(size=1) + ggtitle('Component 3') + 
  geom_ribbon(aes(ymin=mean-qnorm(0.95)*se, ymax=mean+qnorm(0.95)*se, 
                  color=Delivery, fill=Delivery), linetype=2, alpha=0.3) +
  theme(text = element_text(size = 15), axis.title.y = element_blank())

ggarrange(p_feat_obs2, p_feat_obs3 ,nrow=2,common.legend=TRUE,legend = 'bottom')


p_feat_obs3 <- ggplot(data=tab_feat_obs[tab_feat_obs$PC=='PC3',], 
                      aes(x=time, y=value, group=studyid, color=delivery)) + 
  geom_line() + ggtitle('Aggregated trajectory by 3rd ') +
  xlab('days')
## save as 6-by-4 inch
...
##################
rm(list=ls())
setwd('/Users/hrgst/Dropbox/open problem/functional_tensor/code')
...
crops.est = FTSVD(crops.tensor, f.grid = c(list(NULL),list(NULL),list((1:59)/59)), 
                  rank=2, L=50, penalty = 1e-5, initialization = "Spectral")
